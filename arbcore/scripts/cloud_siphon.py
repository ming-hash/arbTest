    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=15) as response:
            ...
            for line in content.strip().split('\n'):
                ...
                # var hq_str_hf_MGC="微黄金,..."
                symbol = line.split('=')[0].split('_')[-1]
                parts = line.split('"')[1].split(',')
                if len(parts) >= 14:
                    # 昨收盘价(最新价)在索引0，昨结算价在索引 7
                    close_price = float(parts[0])
                    settle = float(parts[7])
                    futures_data.append({"symbol": symbol, "settle": settle, "close": close_price})
            
            if futures_data:
                logger.info(f"✅ 期货结算价获取成功: {futures_data}")
                return {"date": datetime.now().strftime('%Y-%m-%d'), "data": futures_data}
    except Exception as e:
...
def main():
    ...
    futures_data = fetch_sina_futures()
    if futures_data:
        path = os.path.join(args.outdir, f"futures_{today_str}.json")
        with open(path, 'w', encoding='utf-8') as f:
            json.dump(futures_data, f, ensure_ascii=False, indent=2)
        logger.info(f"💾 期货数据已保存")
