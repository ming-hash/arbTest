...
### 8.3 Master-Slave 架构（通达信 DLL 多开冲突保护）

- **Master（主系统 - LOFarb）**：占用本地端口 5000 (Flask)，独占通达信 TQ 接口
- **Slave（从系统 - ArbDashboard 看板）**：启动时自动探测 127.0.0.1:5000 端口
  - 如果端口 5000 已被占用 → 自动进入 Slave 模式（禁用 TDX 和实盘下单）
  - 如果端口 5000 未占用 → 独立运行，直接绑定本地 TDX 驱动
